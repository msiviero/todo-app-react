...
export const TodoList = ({ api, keyGenerator }: TodoListProps) => {
        ...
        api.getTodos().then((result) => {
            if (!result.ok) {
                return dispatchNotification({
                    type: 'SHOW',
                    message: `Error getting todos: ${result.cause}`,
    ...
    }, [api, dispatchNotification, dispatchTodoAction]);
    ...
    const onTodoDelete = async (todo: Todo) => {
        try {
            ...
            dispatchTodoAction({ type: 'REMOVE_TODO', item: todo })
            dispatchNotification({
                type: 'SHOW',
                ...
                <div className="bg-white rounded-xl shadow-xl overflow-hidden" role="table">
                    {todos.map((todo) => (
                        <div className="flex flex-row gap-4 space-between items-center justify-center border-gray-100 border-b hover:bg-gray-50" key={todo.key} role="row">
                            ...
                            <div role="cell">
                                <button aria-label="Delete todo" onClick={() => onTodoDelete(todo)} className="text-red-500 hover:bg-red-50 p-4 cursor-pointer">
