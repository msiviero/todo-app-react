...
export const Checkbox = ({ label, checked }: CheckboxProps) => {
    ...
    return (
        <div className="flex items-center">
            <input id={`checkbox_${label}`} onChange={handleCheckboxChange} name={label} checked={isChecked} type="checkbox" className="mx-4 appearance-none peer w-7 h-7 border-2 border-gray-300 rounded-sm bg-white" />
            {
                label ? <label className={`text-lg cursor-pointer ${isChecked ? 'line-through' : ''}`} htmlFor={`checkbox_${label}`}>{label}</label> : null
            }
